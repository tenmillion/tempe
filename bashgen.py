...
for i in range(-5, 2+1) : # 2**-2.5 to 2**1 times original bp wts
	f = open('batch-auto'+str(i+5+1)+'.sh', 'w')
	for j in range(10/2,36/2+1) : # 10 to 36 deg c, in 2 degC steps
		for k in range(-6,-3+1) : # 2**-3 to 2**-1.5 times original pp wts
			cmd='~/nrniv -c "degc=' + str(j*2) + '" -c "bp=' + str(2**(i/2.0)) + '" -c "pp=' + str(2**(k/2.0)) + '" -c "pb=0.5" -c "ip=0.014" init.hoc\n'
			f.write(cmd)
			cmd='~/nrniv -c "degc=' + str(j*2) + '" -c "bp=' + str(2**(i/2.0)) + '" -c "pp=' + str(2**(k/2.0)) + '" -c "pb=0.25" -c "ip=0.014" init.hoc\n'
			f.write(cmd)
	f.close()
# f.close()
